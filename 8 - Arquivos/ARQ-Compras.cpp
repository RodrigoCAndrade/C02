/*
    ...
    @date: 09/06/2025
    @description: This program is an exercise from my C02 classes (ARQ - Compras).
    @version: 1.0
    @license: MIT
    @language: C++
...
// @description: Struct for storing the products
struct dados {
    string produto;
    float preco;
};
...
int main() {
    ...
    // Searching for the bought product in the products array and calculating the total price
    for (int i = 0; i < product_index; i++) {
        if (products[i].produto == bought_product_name) {
            cout << "R$ " << fixed << setprecision(2) << products[i].preco * bought_product_amount << endl;
            break;
        }
    }
